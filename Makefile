...
log-tcp-rfc5424:
	echo "$(shell bash -c "echo '<$${RANDOM:1:3}>1 ${date_rfc5424}'") my-wonderful-root-server.example.com random-app-name 1337 ID47 [exampleSDID@32473 iut="3" eventSource="Application" eventID="1011"] Service Broker Log - TCP unencrypted - RFC5424" | ${netcat} ${netcat_flags} ${host} ${tcp_port}

# UDP
log-udp-rfc3164:
	echo "$(shell bash -c "echo '<$${RANDOM:1:2}>${date_rfc3164}'") my-wonderful-root-server su: 'su root' failed for vcap on /dev/pts/0 -- UDP -- RFC3164" | ${netcat} ${netcat_flags_udp} ${host} ${udp_port}

log-udp-rfc5424:
	echo "$(shell bash -c "echo '<$${RANDOM:1:3}>1 ${date_rfc5424}'") my-wonderful-root-server.example.com random-app-name 1337 ID47 [exampleSDID@32473 iut="3" eventSource="Application" eventID="1011"] Service Broker Log - UDP - RFC5424" | ${netcat} ${netcat_flags_udp} ${host} ${udp_port}
